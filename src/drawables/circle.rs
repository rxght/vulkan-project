...
use vulkano::{
    buffer::BufferContents, pipeline::graphics::{color_blend::ColorBlendState, vertex_input::Vertex}, shader::ShaderStages,
};

use crate::{graphics::{
    bindable::{self, PushConstant}, drawable::{DrawableEntry, GenericDrawable}, pipeline::PipelineBuilder, shaders::{frag_circle, vert_circle}, Graphics
...
impl Circle {
    ...
    pub fn update_transform(&self) {
        self.transform.access_data(|data| {
            ...
            data.transform = (
                cgmath::Matrix4::from_translation([pos.x, pos.y, 0.0].into())
        ...
        match self.is_grabbed.get() {
            false => {
                if input.mouse.is_button_pressed(1) {
                    let distance = input.mouse.cursor_position.get().distance(self.position.get());
...
fn shared_bindables(gfx: &Graphics) -> Vec<Arc<dyn bindable::Bindable>>
{
    ...
    vec![
        ...
        bindable::VertexShader::from_module(
            vert_circle::load(gfx.get_device()).unwrap(),
        ),
        bindable::FragmentShader::from_module(
            frag_circle::load(gfx.get_device()).unwrap(),
        ),
